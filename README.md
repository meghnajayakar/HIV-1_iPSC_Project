...
> **Note:** `CRAM_selected/` should contain symlinks (not copies) to the selected extreme cell line CRAM files, e.g. `ln -s /path/to/original.cram CRAM_selected/`. The `ref/` directory should be populated with the GRCh38 reference FASTA, its `.fai` index, and the sequence dictionary (`.dict`) before running the per-sample gVCF step. `software/` should contain the Singularity VEP image and the corresponding `vep_data` cache directory.
